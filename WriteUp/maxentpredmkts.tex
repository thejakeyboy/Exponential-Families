...
\noindent
Given a proper scoring rule $S$ any affine transformation $\tilde{S}(\mu,x) = aS(\mu,x) + b(x)$ of the rule, with $a > 0$ and $b$ an arbitrary real-valued function of the outcomes, again yields a proper scoring rule termed \emph{equivalent}~\citep{Dawid98,GneitingRa07}. Throughout we will implicitly apply such affine transformations to obtain the clearest version of the scoring rule. We will also focus on scoring rules where inequality~(\ref{eq-proper}) is strict to avoid trivial cases such as constant scoring rules.
...
\end{eqnarray}
%
The latter is positive by the strict convexity of $T$, which shows that the log score is proper. For the converse, assume the defined log score is proper. By the Savage characterization of proper scoring rules for expectations (see~\citet[Thm.\ 1]{GneitingRa07} and~\citet{Savage71}), we must have
%
$$
...
There is a well-known duality between scoring rules and cost-function based markets~\citep{acv13,hanson03}. To see this in our context, recall from~(\ref{equalizer-rule}) that
%
\begin{equation*}
...
<<<<<<< HEAD
\subsection{Behavior of Traders endowed with Exponential Utility}
\sindhu{\cite{norstad99util} for instance?}
...
=======
\section{Risk-Averse Traders with Exponential Utility}
...
>>>>>>> 90b5e45c1c4721e657623140330fb9b51d5a0f22
%
