...
The aggregation properties of market mechanisms have also been explored from a machine learning perspective in~\cite{storkey11}. 
...
The following observation is crucial. Let $\tp \in \mP$ be a density (not necessarily from an exponential family) with expected statistic $\mu$, let $p(\cdot\,;\mu)$ be the exponential family with the same expected statistic, and let $\hmu \in \mM$ be an alternative report. Then from~(\ref{exp-fam}) we have
%
...
\end{eqnarray*}¥
A trader who adjusts the market state may expect positive profit of at least $\lambda D_{T}(\theta,\htheta)$.
 \end{proof}
...
% Acknowledgments
\begin{acks}
We thank Rafael Frongillo and the anonymous referees for valuable feedback.
\end{acks}
