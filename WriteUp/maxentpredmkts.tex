...
\section{Risk Aversion, Exponential Utility, and Market Equilibrium}
...
\end{eqnarray*}
This follows from Proposition \ref{prop:exp-util} \jake{Sindhu this ref isn't in the doc}. The maximizer is $\delta_{2}^* = (\Theta - \theta') / (1+a)$, which moves the share vector to $\theta' + \delta_{2}^* = \frac{1}{1+a} \Theta + \frac{a}{1+a} \theta'$ which is a convex combination of the effective belief and the current market state.
...
Define a potential function as  $$\Phi(\vec{\delta})\defeq  T(\theta+\sum_{i}\delta_{i})+\sum_{i}\frac{1}{a_{i}}T(\htheta_{i}-a_{i}\delta_{i})$$
