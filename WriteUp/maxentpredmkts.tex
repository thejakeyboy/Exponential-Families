...
RAHUL SAMI
\affil{University of Michigan, Ann Arbor}\footnote{Current affiliation: Google, Inc. This work was done while the author was at the University of Michigan.}}
%%\author{GANG ZHOU
...
Some commonly studied distributions that are in fact exponential families include the binomial, beta and normal distributions. To see for instance, that the normal distribution is an exponential family note that instead of the usual parametrization using mean  $\mu$ and  variance $\sigma$, it can be parametrized in terms of $\vec{\theta}=(\theta_{1},\theta_{2})$ where $\theta_{1}=\frac{\mu}{\sigma^{2}}$ and $\theta_{2}=\frac{-1}{2\sigma^{2}}$,  with corresponding log partition function $-\frac{\beta_{1}^{2}}{4\beta_{2}}-\frac{1}{2}\log(-2\beta_{2})$.
...
We assume that the traders trade in multiple instances of the market. That is, after the initial setup and trades, the outcome is revealed and the traders are paid off. Then, the market runs again with the same set of traders and the outcome drawn from a (possibly different) exponential family distribution. The goal of this section is to analyze how the budget of traders evolve under these circumstances. As before, the market price is interpreted as a probability density over the outcome space and the share vector as the natural parameter of an exponential family distribution. Consistent with the connections drawn in Section~\ref{sec:scoring} and throughout, we measure the error in prediction using the standard log loss.  
