...
%-literature review
%- exp family review [Sindhu; Tuesday]
...
% 	[- rate of convergence?]
%- budgets and adversarial markets [Sindhu; Monday]
...
In this section, we assume that the traders have exponential family beliefs. The cost function has the same form as the log partition function $\psi$ of the exponential family and the payoff is determined by the sufficient statistics of the data $\phi(x)$.
...
\end {theorem}
\begin{proof}
Let the cost function $C$ be equal to the log partition function $\psi$ of the belief distribution. The payoff is given by the sufficient statistics $\phi(x)$. Then, the trader's expected net payoff is given by
\begin{eqnarray*}
&& \E_{x\sim P_{\htheta}}[C(\theta)-C(\theta')-(\theta-\theta')\phi(x)]\\
&=& \psi(\theta)-\theta\E_{x\sim P_{\htheta}}[\phi(x)]-(\psi(\theta')-\theta' \E_{x\sim P_{\htheta}}[\phi(x)])\\
&=& \psi(\theta)-\theta\nabla\psi(\htheta)-(\psi(\theta')-\theta' \nabla\psi(\htheta))\\
&=& \psi(\theta)-\psi(\htheta)-\nabla\psi(\htheta)(\theta-\htheta)-(\psi(\theta')-\psi(\htheta)- \nabla\psi(\htheta)(\theta'-\htheta))\\
&=& D_{\psi}(\theta,\htheta)-D_{\psi}(\theta',\htheta)\\
&\geq&\lambda D_{\psi}(\theta,\htheta)\geq0
\end{eqnarray*}
The second to last inequality holds since $D_{\psi}(\theta',\htheta)$ is convex in $\theta'$ and we have:
\begin{eqnarray*}
D_{\psi}(\theta',\htheta)&=&D_{\psi}\left(\lambda\htheta+(1-\lambda)\theta,\htheta\right)\\
&\leq&\lambda D_{\psi}( \htheta ,\htheta)+ (1-\lambda) D_{\psi}( \theta ,\htheta)\\
&=&(1-\lambda) D_{\psi}( \theta ,\htheta)
%\implies D_{\psi}(\theta',\beta)+aD_{\psi}(\theta',\beta)&\leq& aD_{\psi}( \theta ,\beta)\\
\end{eqnarray*}¥
Thus, a trader who moves the market state can expect his profit to be positive and at least $\lambda D_{\psi}(\theta,\htheta)$.
 \end{proof}
